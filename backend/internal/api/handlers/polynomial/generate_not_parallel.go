...
func generateNotParallel(c *gin.Context, matrices [][]float64, coefficients []float64, identityMatrix [][]float64, workerClient proto.WorkerServiceClient, client *entity.WorkersClient) error {
	log.Printf("Starting parallel calculation with matrix size: %d", len(matrices))

	stream, err := workerClient.GetPolynomialCalculation(c)
	if err != nil {
