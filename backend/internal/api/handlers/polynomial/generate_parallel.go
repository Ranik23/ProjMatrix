...
func generateParallel(c *gin.Context, matrices [][]float64, coefficients []float64, identityMatrix [][]float64, workerClient proto.WorkerServiceClient, client *entity.WorkersClient) error {
	log.Printf("Starting parallel calculation with matrix size: %d", len(matrices))

	stream, err := workerClient.GetParallelPolynomialCalculation(c)
	if err != nil {
	...
	result := entity.CalculationResult{
		OperationType: "Parallel Calc Polynomial",
		TimeCalc:      resp.Time,
	}
